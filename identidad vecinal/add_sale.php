...
<?php
    ...
    validate_fields($req_fields);
        if(empty($errors)){
          ...
          $s_dest    = $db->escape($_POST['sale_price']);
          $s_qty     = $db->escape((int)$_POST['quantity']);
          $s_price  = $db->escape($_POST['buy_price']);
          $date      = $db->escape($_POST['date']);
          $s_date    = make_date();

          $sql  = "INSERT INTO sales (";
          $sql .= " product_id,dest,qty,price,date";
          $sql .= ") VALUES (";
          $sql .= "'{$p_id}','{$s_dest}','{$s_qty}','{$s_price}','{$date}'";
          $sql .= ")";

                if($db->query($sql)){
                  update_product_qty($s_qty,$p_id,$s_price);
